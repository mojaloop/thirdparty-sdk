...
const buildMsg = (verb, msg, data, id = generateSlug(4)) =>
  serialise({
    verb,
    msg,
    data,
  ...
  const config = {
    outbound: {
      ...
      tls: {
        ...
        creds: {
          ca: 'newwwwwwwwwwwwwwwwwwwww',
          cert: 'newwwwwwwwwwwwwwwwwwwww',
          key: 'newwwwwwwwwwwwwwwwwwwww'
        }
      }
    }
  }

  const updateConfMsg = build.CONFIGURATION.NOTIFY(config)
  const sendToAllClients = (msg) =>
    Promise.all(
      [...server.clients.values()].map((socket) =>
        new Promise((resolve) => socket.send(msg, resolve)).catch(console.log(socket, msg))
      )
    )
  await sendToAllClients(updateConfMsg)
